...
class SEOBlogGenerator:
    ...
    def _safe_format_blog_post(self, blog_post_data):
        ...
        def _normalize_model_output_to_markdown(self, raw_text: str) -> str:
            """
            Normalize model output into plain markdown/text.

            Steps:
            - Strip surrounding code fences (``` or ```json)
            - Attempt to extract JSON object and pull 'final_article' or the largest string value
            - If JSON extraction fails, locate the first markdown heading and return from there
            - Fallback to returning the cleaned text
            """
            try:
                if not raw_text:
                    return ''
                ...
                # Try to extract JSON blob inside the text
                json_blob = None
                ...
                def _iterate_strings_in_json(obj):
                    if isinstance(obj, str):
                        yield obj
                    elif isinstance(obj, dict):
                        for v in obj.values():
                            for s in _iterate_strings_in_json(v):
                                yield s
                    elif isinstance(obj, list):
                        for item in obj:
                            for s in _iterate_strings_in_json(item):
                                yield s
                    else:
                        return
                ...
                if json_blob:
                    try:
                        ...
                        if isinstance(parsed, dict):
                            # Prefer explicit final_article
                            if 'final_article' in parsed and isinstance(parsed['final_article'], str):
                                return parsed['final_article'].strip()
                            # Nested blog_post.content
                            if 'blog_post' in parsed and isinstance(parsed['blog_post'], dict):
                                bp = parsed['blog_post']
                                if 'content' in bp and isinstance(bp['content'], str):
                                    return bp['content'].strip()
                                if 'final_article' in bp and isinstance(bp['final_article'], str):
                                    return bp['final_article'].strip()

                            # Otherwise pick the longest string leaf in the JSON
                            candidates = [s for s in _iterate_strings_in_json(parsed) if isinstance(s, str)]
                            if candidates:
                ...
                # If no JSON, try to find first markdown heading and return from there
                if text.startswith('# '):
                    return text
                idx = text.find('\n# ')
                if idx != -1:
                    # Return from the first main heading found
                    return text[idx+1:].strip() if text[idx+1:].startswith('#') else text[idx:].strip()

                # As a last resort, return cleaned text
                return text
            except Exception:
    ...
    def _generate_simple_blog_post(self, target_keyword, content_gaps):
        ...
        llm = ChatGoogleGenerativeAI(
            ...
            google_api_key=os.getenv("GOOGLE_GEMINI_API_KEY"),
            temperature=0.3,
            max_tokens=8192  # Increased token limit to reduce truncation
        )
        
        system_prompt = """You are an expert blog writer. Create a comprehensive, SEO-optimized blog post.
        
        Return JSON format (MUST include `final_article` key containing the full article in markdown):
        {
            "blog_post": {
                "title": "SEO optimized title",
                "word_count": number
            },
            "final_article": "Full blog post content in markdown",
            "seo_meta": {
        ...
        Only output valid JSON. Do NOT include any additional commentary or text outside the JSON object.
        """
        ...
        try:
            ...
            try:
                ...
                # If model returned a final_article key, normalize into blog_post.content
                if isinstance(result, dict) and 'final_article' in result:
                    final = result.get('final_article') or ''
                    if 'blog_post' not in result:
                        result['blog_post'] = { 'title': f"Complete Guide to {target_keyword}", 'word_count': len(final.split()) }
                    result['blog_post']['content'] = final
                    result['blog_post']['word_count'] = len(final.split())
                return result
    ...
    def _ultra_minimal_blog_generation(self, target_keyword, secondary_keywords=None, blog_outline=None, target_length=None):
        ...
        try:
            ...
            llm = ChatGoogleGenerativeAI(
                ...
                google_api_key=os.getenv("GOOGLE_GEMINI_API_KEY"),
                temperature=0.3,
                max_tokens=4096,  # Increased slightly to reduce truncation while keeping memory efficiency
            ...
            # Ultra-minimal system prompt: require a JSON response with final_article
            system_prompt = f"""You are an expert blog writer. Return ONLY a JSON object containing a 'final_article' key whose value is the full article in markdown.

Use this EXACT title: "{target_keyword}". The JSON must look like:
{{
  "blog_post": {{ "title": "{target_keyword}", "word_count": number }},
  "final_article": "Full article in markdown",
  "seo_meta": {{ "meta_title": "...", "meta_description": "..." }}
}}

STRICT RULES:
- Only output JSON and nothing else (no commentary or extra text).
- final_article must contain the full blog post in markdown, starting with '# {target_keyword}' as the first line.
- Follow any outline provided in the Suggested Outline context and cover each section.
- Avoid using asterisks (*) in headings or section titles.
"""

            # Human prompt: provide context and require the JSON object
            human_prompt = f"""Create a comprehensive blog post using the outline and sections provided below. Include the following context:

{user_context}

Return ONLY the JSON object described in the system prompt. The 'final_article' value should be the full article in markdown and must follow the outline's sections. Do NOT include any text outside the JSON."""
            ...
            # Parse response - try JSON first (expects 'final_article'), otherwise treat as markdown
            try:
                ...
                # Attempt to load JSON if possible
                ...
                try:
                    ...
                    # If parsed JSON contains final_article, use it
                    if isinstance(parsed, dict) and 'final_article' in parsed:
                        response_content = parsed.get('final_article') or ''
                    elif isinstance(parsed, dict) and 'blog_post' in parsed and isinstance(parsed['blog_post'], dict) and 'content' in parsed['blog_post']:
                        response_content = parsed['blog_post']['content']
                    else:
                        # Fallback to stringified JSON if structure unexpected
                        response_content = json.dumps(parsed)
                except Exception:
                    # Not JSON - use raw text as markdown
                    response_content = raw
...
Remember to practice regularly and stay committed to continuous learning."""
    ...
    def _regenerate_final_article(self, title: str, existing_content: str, target_word_count: int = None) -> str:
        """Attempt to regenerate a final article using the LLM when earlier outputs were outlines or analysis.

        Returns the article as a string, or empty string on failure.
        """
        try:
            if not os.getenv('GOOGLE_GEMINI_API_KEY'):
                logger.warning("GOOGLE_GEMINI_API_KEY not set; cannot regenerate final article")
                return ''
            ...
            model = "gemini-2.5-flash"
            # If a larger model is allowed, prefer it; fall back to flash
            try:
                model = os.getenv('PREFERRED_GEMINI_MODEL', model)
            except Exception:
                pass
            ...
            wc_text = f" target of approximately {target_word_count} words" if target_word_count else ""

            system_prompt = (
                "You are an expert content writer. Return ONLY the final blog article body in clean markdown format. "
                "Do NOT include any JSON, analysis, or comments. Start with the main title as a single '# ' heading, then the content."
            )

            human_prompt = (
                f"Write a polished, SEO-optimized blog post with this EXACT title: \"{title}\".{wc_text} "
                "Do NOT output any metadata, JSON, or explanations. Output only the article in markdown."
            )
            ...
            logger.error(f"Error during final article regeneration: {e}")
            return ''
    ...
    def _generate_enhanced_blog_post(self, outline_data, analysis_results, target_length):
        ...
        # Get comprehensive tool links for context only when appropriate
        # Avoid biasing the model toward tool/comparison content unless the
        # title or outline explicitly implies a tools comparison or the
        # outline_data contains an explicit flag.
        should_include_tools = False
        title_lower = (analysis_results.get('target_keyword') or '').lower()
        # If outline_data explicitly requests tool links, respect it
        if isinstance(outline_data, dict) and outline_data.get('include_tool_links'):
            should_include_tools = True
        # If the provided title explicitly mentions comparison keywords, include tools
        if any(k in title_lower for k in [' vs ', ' vs. ', 'compare', 'comparison', 'best tools', 'tool comparison']):
            should_include_tools = True

        tool_links_context = self._get_tool_links_context() if should_include_tools else ''
        ...
        # Build optional tool links section only if we should include tools
        tool_links_section = f"AVAILABLE TOOL LINKS (use these when mentioning tools):\n{tool_links_context}" if tool_links_context else ""
        ...
        # NOTE: Do NOT introduce or favor tool lists, content-marketing examples,
        # or tool comparisons unless the title or outline explicitly asks for them.
        # Only include tool links when requested or when the title is a comparison.
        {tool_links_section}

        Return JSON (MUST include `final_article` key containing the full article in markdown):
        {{
            "blog_post": {{
                "title": "{provided_title}",
                "word_count": "actual word count"
            }},
            "final_article": "Full blog content in markdown (use headings and sections exactly as requested)",
            "seo_meta": {{
        ...
        IMPORTANT: Only return valid JSON. Do NOT include extra commentary or text outside the JSON object.
        """
        ...
        try:
            ...
            try:
                ...
                # If the model provided final_article, prefer that as the authoritative article content
                if isinstance(blog_data, dict) and 'final_article' in blog_data:
                    final = blog_data.get('final_article') or ''
                    if 'blog_post' not in blog_data:
                        blog_data['blog_post'] = {'title': provided_title}
                    blog_data['blog_post']['content'] = final
                    blog_data['blog_post']['word_count'] = str(len(final.split())) + ' words'
                else:
                    # Backwards compatibility: if blog_post.content exists, compute its word count
                ...
                try:
                    logger.info(f"Generated blog with {blog_data.get('blog_post',{}).get('word_count')} (target: {target_word_count})")
                except Exception:
                    pass
